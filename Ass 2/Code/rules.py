...
class around:
    def __init__(self, threshold, lb, ub):
        self.params = np.array([threshold])
        self.lob = lb+ 0.1*(ub-lb)
        self.uob = ub- 0.1*(ub-lb)

    def Lboundary(self):
        return (self.uob - self.lob)*self.params[0] + self.lob- 0.1*(self.uob-self.lob)
    def Hboundary(self):
        return (self.uob - self.lob)*self.params[0] + self.lob+ 0.1*(self.uob-self.lob)

    def __call__(self, value):
        return (value <= self.Hboundary() and value >= self.Lboundary())
...
class between:
    ...
    def __init__(self, threshold1,threshold2, lb, ub):
        self.params = np.array([min(threshold1,threshold2), max(threshold1,threshold2)])
        self.lb = lb
        self.ub = ub
    ...
    def Lboundary(self):
        return (self.ub - self.lb)*self.params[0] + self.lb
    def Hboundary(self):
        return (self.ub - self.lb)*self.params[1] + self.lb

    def __call__(self, value):
        return (value <= self.Hboundary() and value >= self.Lboundary())
    
    def __str__(self):
        return ">= " + str(self.Lboundary()) +"and <= " + str(self.Hboundary())
...
class notBetween:
    ...
    def __call__(self, value):
        return (not (value <= self.Hboundary()) and not (value >= self.Lboundary()))
...
def create_rule(num_attrs, num_classes, lb, ub):
    ...
    for i in range(num_attrs):
        ...
        #elif r < 0.5:
          #   conditions.append(around(random.random(), lb[i], ub[i]))
        if r<0.30:
             conditions.append(between(random.random(),random.random(),lb[i], ub[i]))
        elif r<0.61:
            conditions.append(notBetween(random.random(),random.random(),lb[i], ub[i]))
        else:
...
if __name__ == '__main__':
    ...
    utils.plot_experiment(evals, lower, mean, upper, legend_name = 'Classification Rules')
    plt.legend()
    plt.show()